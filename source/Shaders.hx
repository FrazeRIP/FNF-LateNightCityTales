...
class GlitchHardcoreEffect extends Effect{
...
class GlitchHardcoreShader extends FlxShader{
	@:glFragmentSource('
    #pragma header
	//https://www.shadertoy.com/view/4t3yRn
	...
	void main()
	{
    	...
    	float rnd = rand(vec2(iTime)) * uStrength;
    
    if (mod( openfl_TextureCoordv.y, rnd) > rnd / 8.0)
    {
        float of = sin(openfl_TextureCoordv.y / rnd * iTime) * rnd * 2.0;
        uv = vec2(openfl_TextureCoordv.x + of, openfl_TextureCoordv.y + of / 2.0);
        gl_FragColor = texture2D(bitmap, uv);
        gl_FragColor.r *= texture2D(bitmap, uv + of * rnd * vec2(0.5, 0.0)).r;
        gl_FragColor.g *= texture2D(bitmap, uv).g;
        gl_FragColor.b *= texture2D(bitmap, uv - of * rnd * vec2(0.5, 0.0)).b;
        //gl_FragColor *= 1.5;
    } else
    {
        uv = vec2(openfl_TextureCoordv.x - rnd * 4.0, openfl_TextureCoordv.y + rnd / 2.0);
        gl_FragColor = texture2D(bitmap, uv);
        gl_FragColor.r *= texture2D(bitmap, uv + rnd * vec2(0.5, 0.0)).r;
        gl_FragColor.g *= texture2D(bitmap, uv).g;
        gl_FragColor.b *= texture2D(bitmap, uv - rnd * vec2(0.5, 0.0)).b;
        //gl_FragColor *= 1.5;
    }
}
	')
...
class DistortedTVShader extends FlxShader{
	@:glFragmentSource('
		#pragma header

		//https://www.shadertoy.com/view/ldXGW4 
		// change these values to 0.0 to turn off individual effects
		uniform float vertJerkOpt;		//Screen randon jumps like brrrrr
		uniform float vertMovementOpt;	//Screen random smooth vertically move
		uniform float bottomStaticOpt;	//White stuff cames out from the button
		uniform float scalinesOpt;		//Scanline baby
		uniform float rgbOffsetOpt;		//Chromatic Aberration baby
		uniform float horzFuzzOpt;		//Glitchhhhhhhhhhhh
...
void main( )
{
	...
	float red 	=   texture(	bitmap, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
	float green = 	texture(	bitmap, 	vec2(uv.x + xOffset,	  y)).g+staticVal;
	float blue 	=	texture(	bitmap, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
